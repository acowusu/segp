...
export const TimeFrame: React.FC<TimeFrameProps> = ({action, row, data, deleteItem, setToReplace, toReplace}) => {
    return ( 
        <ContextMenu>
            <ContextMenuTrigger>
            <div key={row.id} className={`flex flex-row items-center cursor-pointer border ${toReplace && toReplace.id === action.id ? "border-blue-400 border-2" : "border-white"} h-full`}>
            ...
            <ContextMenuContent>
                ...
                <ContextMenuItem onClick={() => {
                    ...
                    }}>{toReplace && toReplace.id === action.id ? "Deselect" : "Replace"}</ContextMenuItem>
